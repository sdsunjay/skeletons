...
int read_lines(const char * fname, int (*call_back)(const char*, const char*))
{
    ...
    while (1) {
        ...
        /* Call the call back and check error indication
         * Announce error here, because we didn't tell call_back the file name
         */
...
int main(int argc, char* argv[])
{
    ...
    getSpace();
    returnValue = read_lines(argv[1], print_line);
    if(returnValue==1)
    {
        printWords();
    }
